...
End-to-end tests I wrote for this app, driving a real browser against a running instance of it, the same approach I use across the other test-automation repos in this profile.
...
- `test_category_flow.py`, the post-login category picker (Hair for Women, Hair for Men, Makeup, Skincare), persistence across reload, the real Makeup and Skincare catalogs (5 products each, with embedded video tutorials), switching categories without losing the cart
...
- `test_pricing.py`, money/rounding correctness on the "welcome discount" (10% off a budget quote): a parametrized sweep across the full budget range proving the displayed discount and post-discount total always sum back to the original (regression test for a real off-by-€1 rounding bug found and fixed while writing this suite), plus boundary, NaN/undefined-defensive, and currency-conversion cases
- `test_concurrency.py`, real two-session concurrency (two independent, already-logged-in browser sessions firing requests at the same instant via `ThreadPoolExecutor`, not one after another): two different people double-clicking the same booking slot, two different people booking different slots at once, and two concurrent signups for the same e-mail
...
- Most tests drive the UI through Selenium; a few (`test_api_errors.py`, the double-booking case in `test_booking.py`, and all of `test_concurrency.py`) call `fetch()` directly from inside the authenticated browser session to exercise server-side validation and edge cases that aren't reachable purely through clicks (a malformed request body, a race on the same booking slot), still an in-browser, session-authenticated call, not a separate HTTP client. `test_concurrency.py` additionally uses a second, fully independent browser session (`second_driver` fixture) plus `ThreadPoolExecutor` so two requests are genuinely in flight together, simulating two different people rather than one person double-clicking.
