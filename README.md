...
I built this as a functional prototype to validate the business logic before putting any real money into a full product. It's a fictional hair/beauty diagnostic and booking app, and it covers: client accounts (login/password), a post-login category picker (Hair for Women, Hair for Men, Makeup, Skincare, all sharing one account and one cart), hair anamnesis, harmony assessment (visagism + personal color analysis), a full protocol menu with no budget gating, curated Makeup and Skincare catalogs grounded in researched 2026 K-beauty trends, an optional budget-capped quote, a technical data sheet per product (usage, ingredients, consumption plan, embedded video tutorial), a lightweight gamification layer (points, progress bar, achievement badges), appointment booking with a partner professional, and product orders (no real payment processing).
...
- Post-login category picker (Hair for Women, Hair for Men, Makeup, Skincare), all four route to the same account and the same shopping cart; switch between them anytime from the pill bar without losing anything
...
- Real AI engine (today the harmony/protocol responses are deterministic rules, not calls to a language model; a future version could plug in an LLM for that)
...
- Handling real client data in production would also require a full LGPD/GDPR and Anvisa/INFARMED compliance review (data retention, consent, right to erasure), not yet addressed in this prototype
